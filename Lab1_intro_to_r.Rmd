...
```{r plot-girl-trend, eval=FALSE}
library(ggplot2)
ggplot(data = arbuthnot, aes(x = year, y = girls)) + 
  geom_point(col="red")+geom_smooth(method = 'lm', col="purple") +
  ggtitle("Baby Girls")

ggplot(data = arbuthnot, aes(x = year, y = boys)) + 
  geom_point(col="blue")+geom_smooth(method = 'lm', col="navy")+
  ggtitle("Baby Boys")

# intent to plot boys vs girls, thus i need to first combine them together to be anbel to do grouping gain using ggplot and plot girls vs boys

all_birth <-  arbuthnot %>%
  pivot_longer(c(boys,girls), names_to = "Gender", values_to = "Birth")
glimpse(all_birth)

ggplot(data = all_birth, aes(x = year, y = Birth, col=Gender, group=Gender)) + 
  geom_line()+geom_smooth(method = 'lm') +
  ggtitle("Boys vs Girls")

```

## The result interpretation 

Girl-birth rate in general increases over time.
The smooth linear regression shown above supports it.
Additionally, the birth rates for boys and girls follow similar patterns, meaning they increase or decrease together.
Similarly, both increase with the same rate over time.
As a general note, boy-birth rate is more than girl-birth rate, but one can see the same hills and valleys in the data over years
...
```{r plot-proprtion-of-boys-over-time}

ggplot(data = arbuthnot, aes(x = year, y = boy_ratio)) + 
  geom_point(col="black")+geom_line(col="black") +
  ggtitle("Baby boys ratio ")
```
...
the present dataset starts from 1940 and ends in 2002, thus between 1940 to 2002.
It has 63 rows and 3 columns.
You can use colnames to extract the name and thsi dataframe has "year", "boys", and "girls" simialr to the arbuthnot.
...
As a general observation, just comparing boys' birth over girls, it seems boys'' birth rate is higher.
boys' in terms of comparing the ratio of boys_to_girl.
it seems they are close.

```{r compare_boys_girls_between_two_df}
present <- present %>%
  mutate(total = boys+girls)
present <- present %>%
  mutate(boy_to_girl_ratio = boys/girls)
...
The general observation holds true, and boys are born at a higher rate than girls; however, the trends in moving down even though not noticeably.

```{r plot-proprtion-of-boys-over-time-present}

ggplot(data = present, aes(x = year, y = boy_ratio)) + 
  geom_point(col="black")+geom_line(col="black") +
  ggtitle("Baby boys ratio in US between 1940-2002")
```
...
year 1961 has the highest number or total birth with 426826 baby set foot in our world.
